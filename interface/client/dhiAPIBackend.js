...
/**
The backend side of the dhi API.

@method dhiAPIBackend
*/
dhiAPIBackend = function (event) {
    var template = this.template;
    var webview = this.webview;
    var arg = event.args[0];

    // console.trace('dhiAPIBackend event', event);
    ...
    if (event.channel === 'dhiAPI_setBadge') {
        Tabs.update(template.data._id, { $set: {
            badge: arg,
        } });
    }

    if (event.channel === 'dhiAPI_menuChanges' && arg instanceof Array) {
