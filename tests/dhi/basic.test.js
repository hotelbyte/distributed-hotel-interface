...
test['Check for DHI title'] = function* () {
    (yield this.client.getTitle()).should.eql('DHI');
};
...
test['Browser bar should not execute JS'] = function* () { // ETH-01-001
    ...
    const dhi = yield client.execute(() => { return window.dhi; }); // checking if `execute` works
    const pwned = yield client.execute(() => { return window.pwned; });

    should.exist(dhi.value);
...
test['Browse tab should be changed to pinned tab if URLs are the same'] = function* () { // ETH-01-007
    ...
    yield this.navigateTo('https://wallet.hotelbyte.org');
    yield Q.delay(1000);
...
test['Wallet tab shouldn\'t have the page replaced if URLs does not match'] = function* () { // ETH-01-007
    ...
    yield this.navigateTo(`${this.fixtureBaseUrl}index.html?https://wallet.hotelbyte.org`);
    yield client.waitUntil(() => {
...
test['Wallet tab shouldn\'t have the page replaced if URLs does not match - 2'] = function* () { // ETH-01-007
    ...
    yield client.setValue('#url-input', `${this.fixtureBaseUrl}index.html?https://wallet.hotelbyte.org`);
...
//test['Links with target _blank should open inside DHI'] = function* () {
//    const client = this.client;
...
// test['Links with target _popup should open inside DHI'] = function* () {
//     const client = this.client;
//     yield this.navigateTo(`${this.fixtureBaseUrl}/fixture-popup.html`);
//     yield this.getWindowByUrl(e => /fixture-popup.html$/.test(e));
//
...
// ETH-01-005
// test['DHI main webview should not redirect to arbitrary addresses'] = function* () {
//     const client = this.client;
...
// ETH-01-008
test['DHI main webview should not redirect to local files'] = function* () {
    const client = this.client;
