/**
@module preloader DHIUI
*/

// Initialise the Redux store
window.store = require('./rendererStore');

require('./include/common')('dhi');
...
menu.append(new MenuItem({ label: i18n.t('dhi.rightClick.reload'), accelerator: 'Command+R', click() {
    const webview = Helpers.getWebview(LocalStore.get('selectedTab'));
    if (webview) {
        webview.reloadIgnoringCache();
    }
} }));
menu.append(new MenuItem({ label: i18n.t('dhi.rightClick.openDevTools'), click() {
    const webview = Helpers.getWebview(LocalStore.get('selectedTab'));
    if (webview) {
        webview.openDevTools();
    }
} }));
menu.append(new MenuItem({ label: i18n.t('dhi.rightClick.inspectElements'), click() {
    const webview = Helpers.getWebview(LocalStore.get('selectedTab'));
    if (webview) {
