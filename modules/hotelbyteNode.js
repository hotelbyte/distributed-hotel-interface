...
const hotelbyteNodeLog = logger.create('HotelbyteNode');

const DEFAULT_NODE_TYPE = 'ghbc';
...
/**
 * Hotelbyte nodes manager.
 */
class HotelbyteNode extends EventEmitter {
    constructor() {
        super();
    ...
    init() {
        return this._socket.connect(Settings.rpcConnectConfig)
            ...
            .catch(() => {
                hotelbyteNodeLog.warn('Failed to connect to node. Maybe it\'s not running so let\'s start our own...');

                hotelbyteNodeLog.info(`Node type: ${this.defaultNodeType}`);
                hotelbyteNodeLog.info(`Network: ${this.defaultNetwork}`);
                hotelbyteNodeLog.info(`SyncMode: ${this.defaultSyncMode}`);

                // if not, start node yourself
                return this._start(this.defaultNodeType, this.defaultNetwork, this.defaultSyncMode)
                    .catch((err) => {
                        hotelbyteNodeLog.error('Failed to start node', err);
                        throw err;
                    });
            });
    }
    ...
    restart(newType, newNetwork, syncMode) {
        return Q.try(() => {
            ...
            hotelbyteNodeLog.info('Restart node', newType, newNetwork);
            ...
            return this.stop()
                ...
                .catch((err) => {
                    hotelbyteNodeLog.error('Error restarting node', err);
                    throw err;
                });
        });
    }
    ...
    stop() {
        if (!this._stopPromise) {
            return new Q((resolve) => {
                ...
                hotelbyteNodeLog.info(`Stopping existing node: ${this._type} ${this._network}`);
        ...
        hotelbyteNodeLog.debug('Disconnection already in progress, returning Promise.');
     ...
     * @param  {String} nodeType ghbc, eth, etc
     * @param  {String} network  network id
     * @return {Promise}
     */
    _start(nodeType, network, syncMode) {
        hotelbyteNodeLog.info(`Start node: ${nodeType} ${network} ${syncMode}`);

        const isTestNet = (network === 'test');

        if (isTestNet) {
            hotelbyteNodeLog.debug('Node will connect to the test network');
        }
        ...
        return this.stop()
            ...
            .then((proc) => {
                hotelbyteNodeLog.info(`Started node successfully: ${nodeType} ${network} ${syncMode}`);
                ...
                return this._socket.connect(Settings.rpcConnectConfig, {
                    ...
                    .catch((err) => {
                        hotelbyteNodeLog.error('Failed to connect to node', err);
            ...
            .catch((err) => {
                ...
                // if unable to start eth node then write ghbc to defaults
                if (nodeType === 'eth') {
                    Settings.saveUserData('node', 'ghbc');
                }
    ...
    __startNode(nodeType, network, syncMode) {
        ...
        hotelbyteNodeLog.info(`Start node using ${binPath}`);
    ...
    __startProcess(nodeType, network, binPath, _syncMode) {
        ...
        if (nodeType === 'ghbc' && !syncMode) {
            syncMode = 'fast';
        }

        return new Q((resolve, reject) => {
            hotelbyteNodeLog.trace('Rotate log file');

            // rotate the log file
            logRotate(path.join(Settings.userDataPath, 'logs', 'all.log'), { count: 5 }, (err) => {
                if (err) {
                    hotelbyteNodeLog.error('Log rotation problems', err);
                ...
                if (nodeOptions && nodeOptions.length) {
                    hotelbyteNodeLog.debug('Custom node options', nodeOptions);

                    args = args.concat(nodeOptions);
                }

                hotelbyteNodeLog.trace('Spawn', binPath, args);
                ...
                proc.once('error', (error) => {
                    if (STATES.STARTING === this.state) {
                        ...
                        hotelbyteNodeLog.info('Node startup error');
                ...
                proc.stdout.on('data', (data) => {
                    hotelbyteNodeLog.trace('Got stdout data');
                    ...
                    if (STATES.STARTING === this.state) {
                        ...
                        if (nodeType === 'ghbc') {
                            if (dataStr.indexOf('fatal: error') >= 0) {
                                const error = new Error(`Ghbc error: ${dataStr}`);

                                if (dataStr.indexOf('bind') >= 0) {
                                    error.tag = UNABLE_TO_BIND_PORT_ERROR;
                                }

                                hotelbyteNodeLog.debug(error);
                ...
                proc.stderr.on('data', (data) => {
                    hotelbyteNodeLog.trace('Got stderr data');
                ...
                this.once('data', () => {
                    /*
                        ...
                        errors, etc (see ghbc port-binding error above)
                    */
                    setTimeout(() => {
                        if (STATES.STARTING === this.state) {
                            hotelbyteNodeLog.info(`${NODE_START_WAIT_MS}ms elapsed, assuming node started up successfully`);
    ...
    _showNodeErrorDialog(nodeType, network) {
        ...
        if (log) {
            ...
            log = global.i18n.t('dhi.errors.nodeStartup');
        }
        ...
        dialog.showMessageBox({
            type: 'error',
            buttons: ['OK'],
            message: global.i18n.t('dhi.errors.nodeConnect'),
            detail: log,
        }, () => {});
    }
    ...
    _logNodeData(data) {
        ...
        hotelbyteNodeLog.trace(`${nodeType}: ${cleanData}`);
    ...
    _loadDefaults() {
        hotelbyteNodeLog.trace('Load defaults');
        ...
        hotelbyteNodeLog.info(Settings.syncmode, Settings.loadUserData('syncmode'), DEFAULT_SYNCMODE);
        hotelbyteNodeLog.info(`Defaults loaded: ${this.defaultNodeType} ${this.defaultNetwork} ${this.defaultSyncMode}`);
    }
}


HotelbyteNode.STARTING = 0;


module.exports = new HotelbyteNode();
